"""
Experiment: Test Set Ordering Impact on XGBoost Performance
============================================================
Tests whether the order in which projects are introduced to the model
affects its precision, recall, F1, and accuracy.

Cumulative / Online training
---------------------------------------------
Projects are processed in sequence. When project P_i is the test target,
the model is trained ONLY on projects that appeared BEFORE P_i in the
current permutation's order.

  • Permutation 1 (alphabetical): achilles is tested with 0 prior
    projects; activiti with 1 prior; … zxing with 16 priors.
  • Permutation 2 (reversed): zxing is tested with 0 priors; wro4j
    with 1; … achilles with 16 priors.

Because each permutation exposes different projects early vs. late,
the training set composition at each step genuinely differs, so scores
will differ across permutations — especially for projects tested early
(small training set) vs. late (large training set).

SMOTE oversampling
------------------
The flaky-test dataset is heavily imbalanced (~2% flaky). SMOTE is applied
to the training portion of each cumulative fold (never to the validation or
test sets) to balance the minority class before fitting XGBoost.
This can shift precision/recall curves and surface per-project variance that
a model trained on imbalanced data would otherwise suppress.

Run from repo root:
    python -m src.ordering_experiment
"""
...
# 4 permutations — each produces a different cumulative training
# trajectory because early projects = less training data, late = more.
PERMUTATIONS = {
...
def find_best_threshold(model: XGBClassifier, X_val: np.ndarray, y_val: np.ndarray) -> float:
...
def print_delta_table(all_results):
    ...
    print(f"\n{SEP}")
    print("  PER-PROJECT F1 DELTA vs. Permutation 1 (baseline)")
    print("  Positive = scored HIGHER in that permutation for that project.")
    print(SEP)
...
def print_summary(all_results):
    ...
    std_f1 = np.std(f1_avgs)
    if std_f1 < 0.005:
        verdict = "✓  Ordering has NO meaningful impact on macro-average F1."
    elif std_f1 < 0.02:
        verdict = "~  Ordering has a SMALL but detectable impact on macro-average F1."
    else:
        verdict = "✗  Ordering has a NOTABLE impact on macro-average F1 — data volume matters."

    print(f"\n  Verdict: {verdict}")
    print(f"\n  Interpretation:")
    print(f"    Projects tested EARLY (few prior training projects) tend to score lower.")
    print(f"    Projects tested LATE (many prior training projects) benefit from more data.")
    print(f"    The permutation that front-loads data-rich projects will generally score best overall.")

if __name__ == '__main__':
    print("Loading static features...")
    df, feature_cols = load_data(STATIC_FEATURES_CSV)

    available_projects = set(df['project'].unique())
    print(f"  {len(df)} tests | {df['label'].sum()} flaky | {len(available_projects)} projects in dataset")
    print(f"  Using {len(feature_cols)} features\n")

    missing = [p for p in TARGET_PROJECTS if p not in available_projects]
    if missing:
        print(f"  Projects not found in dataset (will be skipped): {missing}\n")

    print("Methodology: CUMULATIVE training + SMOTE + threshold tuning")
    print("  Each project is tested using only the projects that came BEFORE it")
    print("  in the permutation order as training data. SMOTE balances the minority")
    print("  class in each training fold; threshold is tuned on a 20% validation split.")
    print("  Early projects see less training data, so order genuinely affects scores.\n")

    all_results = {}
    for perm_name, project_order in PERMUTATIONS.items():
        print(f"Running: {perm_name} ...")
